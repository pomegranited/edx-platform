...
class MixedModuleStore(ModuleStoreDraftAndPublished, ModuleStoreWriteBase):
    ...
    def get_courses_summary(self, **kwargs):
        ...
        for store in self.modulestores:
            ...
            for course in store.get_courses_summary(**kwargs):
                course_id = self._clean_locator_for_mapping(locator=course['locator'])
                if course_id not in courses:
                    # add location in the course information
                    course['location'] = course_id.make_usage_key('course', 'course')
